...
import { snakeToCamelKeys, toAssetSlug, toAssetSlug2 } from "./helpers";
...
import { OldWhitelisted, OLD_WHITELIST } from "./data/old-whitelist";
...
export async function findTezDexPure(token: any) {
  // token: QSAsset
  ...
  const tokenSlug = toAssetSlug2(token);
